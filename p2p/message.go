...
func (r *eofSignal) Read(buf []byte) (int, error) {
	fmt.Println("(r *eofSignal) Read(buf []byte)")
	if r.count == 0 {
		if r.eof != nil {
			r.eof <- struct{}{}
			r.eof = nil
...
func (p *MsgPipeRW) WriteMsg(msg Msg) error {
	fmt.Println("(p *MsgPipeRW) WriteMsg(msg)")
...
func (p *MsgPipeRW) ReadMsg() (Msg, error) {
	fmt.Println("(p *MsgPipeRW) ReadMsg()")
	if atomic.LoadInt32(p.closed) == 0 {
		select {
		case msg := <-p.r:
			return msg, nil
...
func (self *msgEventer) ReadMsg() (Msg, error) {
	fmt.Println("(self *msgEventer) ReadMsg()")
...
func (self *msgEventer) WriteMsg(msg Msg) error {
	fmt.Println("(self *msgEventer) WriteMsg(msg)")
...
func (self *msgEventer) Close() error {
	fmt.Println("(self *msgEventer) Close()")
